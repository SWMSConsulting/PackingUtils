...
class TestGetBestItem(unittest.TestCase):
    ...
    def test_get_best_item_to_pack_largest_density(self):
        bin = Bin(10, 3, 15)
        packer = PalletierWishPacker(bins=[bin])
        config = PackerConfiguration(
            new_layer_select_strategy=ItemSelectStrategy.LARGEST_DENSITY,
        )
        packer.reset(config)

        items = [
            SingleItem(identifier="item1", width=3, length=1, height=3, weight=15),
            SingleItem(identifier="item2", width=3, length=1, height=3, weight=10),
            SingleItem(identifier="item3", width=3, length=2, height=3, weight=10),
            SingleItem(identifier="item4", width=3, length=3, height=3, weight=10),
        ]
